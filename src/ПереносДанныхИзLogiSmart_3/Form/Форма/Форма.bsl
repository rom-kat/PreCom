...
Процедура ЗаполнитьДанныеДоговорыLSНаСервере()
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		СтрокаТаблицаДоговоры.Контрагент = ПолучитьОбъект(ВыборкаДетальныеЗаписи.ID_Контрагент, "Справочник.Контрагенты");
		СтрокаТаблицаДоговоры.ДоговорКонтрагента = ПолучитьОбъект(ВыборкаДетальныеЗаписи.ID, "Справочник.ДоговорыКонтрагентов");
		СтрокаТаблицаДоговоры.ID_Контрагент = ВыборкаДетальныеЗаписи.ID_Контрагент; 
		СтрокаТаблицаДоговоры.ID_Договор = ВыборкаДетальныеЗаписи.ID;
		СтрокаТаблицаДоговоры.НаименованиеКонтрагентLS = ВыборкаДетальныеЗаписи.Контрагент;
		СтрокаТаблицаДоговоры.ТипДоговора = ВыборкаДетальныеЗаписи.НаименованиеТипаДокумента;
		СтрокаТаблицаДоговоры.НаименованиеДоговорLS = "№ "+ВыборкаДетальныеЗаписи.Номер+" от "+ВыборкаДетальныеЗаписи.Дата_начала_действия;
...
&НаСервереБезКонтекста
Функция СоздатьПаспортНаПеревозку(РеференсЗаказа, СтруктураПаспортаНаПеревозку)
	...
	ПаспортНаПеревозку.Подразделение = СтруктураПаспортаНаПеревозку.ПодразделениеОрганизации;
	ПаспортНаПеревозку.Контрагент = СтруктураПаспортаНаПеревозку.Контрагент;
	ПаспортНаПеревозку.Договор = СтруктураПаспортаНаПеревозку.ДоговорКонтрагента;
...
Функция ПолучитьПаспортНаПеревозку(РеференсЗаказа, СтруктураПаспортаНаПеревозку = Неопределено)
	...
	Если СтруктураПаспортаНаПеревозку = Неопределено Тогда
		...
		Если ВыборкаДетальныеЗаписи.Следующий() Тогда
			...
			СтруктураПаспортаНаПеревозку.ПодразделениеОрганизации = ПолучитьПодразделение();
			СтруктураПаспортаНаПеревозку.Контрагент = ПолучитьОбъект(ВыборкаДетальныеЗаписи.ID_Контрагент, "Справочник.Контрагенты");
			СтруктураПаспортаНаПеревозку.ДоговорКонтрагента = ПолучитьОбъект(ВыборкаДетальныеЗаписи.ID_Договор, "Справочник.ДоговорыКонтрагентов");
...
Функция СоздатьСчет(СтруктураСчета, СчётНаОплатуСсылка)
	...
	Попытка
		...
		СчётТД = СтруктураСчета.ТД;
		
		Для каждого ТС_ТЕ Из СтруктураДанных.ТС_ТЕ Цикл
			СчётТД = СчётТД + Символы.ПС + ТС_ТЕ;
		КонецЦикла;
		...
		СчётНаОплатуОбъект.ПаспортНаПеревозку = ПолучитьПаспортНаПеревозку(СтруктураСчета.РеференсыЗаказов, СтруктураПаспортанаПеревозку);
...
Процедура ЗаполнитьДанныеСчетовLSНаСервере()
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		СтрокаТаблицаСчетов.ВалютаСчёта = ПолучитьВалюта(ВыборкаДетальныеЗаписи.ID_ВалютаKOD);
	...
	Для каждого СтрокаТаблицаУдаленныхСчетов Из ТЗУдаленныхСчетов Цикл
		Если СтрокаТаблицаУдаленныхСчетов.Фл Тогда
			Если МассивСчетов.Найти(СтрокаТаблицаУдаленныхСчетов.Счет)=Неопределено Тогда
				...
				СчетОбъект.Удалить()
			Иначе 	
				ТекстСообщения = ТекстСообщения + Символы.ПС + "" + СтрокаТаблицаУдаленныхСчетов.Счет + " не можеть быть удален из информационной базы тат как найдены объекты ссылающиеся.";
			КонецЕсли; 
		КонецЕсли;
	КонецЦикла;
